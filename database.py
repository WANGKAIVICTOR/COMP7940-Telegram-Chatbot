...
def add_user(username, key):
    ...
    cursor = connection.cursor()  # create the cursor to execute the sql sentence
    cursor.execute(
        """SELECT name,activate_key,times FROM user WHERE activate_key=%s;""", (key))
    ...
    try:
        cursor.execute(
            """INSERT INTO user(is_admin, name) VALUES('%s','%s');""" % (0, username))
        connection.commit()  # save the data
    except Exception as e:
        return False, e
    times = data['times'] - 1
    if(times != 0):
        cursor.execute(
            """UPDATE user SET times=%s WHERE activate_key=%s;""", (times, key))
        connection.commit()  # save the data
    else:
...
def check_user(usernmae, check_admin=False):
    ...
    if check_admin:
        cursor.execute(
            """SELECT * FROM user WHERE name=%s AND is_admin=%s;""", (usernmae, 1))
...
def get_key(usernmae):
    ...
    cursor = connection.cursor()  # create the cursor to execute the sql sentence
    cursor.execute(
        """SELECT activate_key FROM user WHERE name=%s;""", (usernmae))
